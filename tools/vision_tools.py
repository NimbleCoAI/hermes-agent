...
async def _handle_vision_analyze(args: Dict[str, Any], **kw: Any) -> str:
    ...
    result = await vision_analyze_tool(image_url, full_prompt, model, task_id=task_id)

    if not _vision_result_failed(result):
        return result

    # Cascade: the local/aux model failed. If the active main model can read
    # the pixels natively (provider accepts images in tool results), hand the
    # image to it instead of surfacing the aux error.
