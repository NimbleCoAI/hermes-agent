...
def resolve_gateway_approval_in_scope(scope_prefix: str, choice: str,
    ...
    *reason* is the optional free-text explanation from ``/deny <reason>``,
    threaded through so the cross-session admin-deny path relays it to the
    agent just like the same-session deny does.
    """
    ...
    if resolve_all:
        total = 0
        for k in keys:
            total += resolve_gateway_approval(k, choice, resolve_all=True, reason=reason)
        return total
    return resolve_gateway_approval(keys[0], choice, resolve_all=False, reason=reason)
            ...
            if not resolved or choice is None or choice == "deny":
                ...
                return {
                    ...
                    "message": (
                        ...
                        f"action. Do NOT retry this command, do NOT rephrase "
                        f"it, and do NOT attempt the same outcome via a "
                        f"different command. Stop the current workflow and "
