...
class TestApprovalHandlersStayGated:
    """Guard against the upstream-merge landmine: GatewaySlashCommandsMixin
    ships UNGATED _handle_approve_command/_handle_deny_command copies. The
    fork's admin-gated handlers on GatewayRunner shadow them only because the
    class defines its own — if a cleanup ever deletes the run.py copies, the
    mixin's ungated versions take over silently. Pin both facts."""

    def test_runner_shadows_mixin_approve_and_deny(self):
        import inspect
        ...
        for name in ("_handle_approve_command", "_handle_deny_command"):
            runner_fn = GatewayRunner.__dict__.get(name)
            assert runner_fn is not None, (
                f"{name} no longer defined on GatewayRunner — the mixin's "
                "UNGATED copy now handles approvals. Restore the gated handler."
            )
            assert runner_fn is not GatewaySlashCommandsMixin.__dict__.get(name)
            src = inspect.getsource(runner_fn)
            assert "_is_approval_admin" in src, (
                f"GatewayRunner.{name} lost its admin gating "
                "(_is_approval_admin check missing)."
            )
