...
class SessionEntry:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SessionEntry":
        ...
        # the filesystem — interior ``/`` is legitimate (Signal group ids are
        # base64; Google Chat resource names are ``spaces/<id>`` and
        # ``spaces/<id>/threads/<id>``), so it only needs the relaxed guard
        # against genuine traversal vectors.
