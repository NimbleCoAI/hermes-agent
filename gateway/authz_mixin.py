...
class GatewayAuthorizationMixin:
    ...
    def _is_user_authorized(self, source: SessionSource) -> bool:
        ...
        if source.chat_type in {"group", "forum", "channel"} and source.chat_id:
            ...
            # the sender here keeps the gateway consistent with the adapter's
            # decision (otherwise auto-approved groups silently reject their
            # members).
            if source.platform == Platform.SIGNAL:
                _signal_adapter = self._authorization_adapter(
                    Platform.SIGNAL, getattr(source, "profile", None)
                )
